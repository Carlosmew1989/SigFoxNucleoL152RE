...
//  {/* CUT 1.x */
//    uint8_t tmp=0x1E;
//    S2LPSpiWriteRegisters(MCU_CK_CONF_ADDR, 1, &tmp);
//  }
//  else
//  {
//    /* S2LP clock configuration */
//    S2LPGpioClockOutputInit(&(ClockOutputInit){XO_RATIO_1_256, RCO_RATIO_1, EXTRA_CLOCK_CYCLES_0});
//  }
//  
//  /* Instance the structure used to configure the S2LP clock frequency to be divided by a 192 factor. */
//  S2LPGpioInit(&(SGpioInit){S2LP_GPIO_2, S2LP_GPIO_MODE_DIGITAL_OUTPUT_LP, S2LP_GPIO_DIG_OUT_MCU_CLOCK});
  ...
  for(uint32_t i=0;i<2*(RCO_N_SAMPLES+RCO_SETTLING_PERIODS);i++) {
    /* block the routine until the TIM CCP2 IRQ is raised */
    while(!s_xTIMChCompareModeRaised && (cWtchdg!=0xFFFF)) {
    //while(!__HAL_TIM_GET_FLAG(&TimHandle, TIM_FLAG_CC1))/* && (cWtchdg!=0xFFFF))*/{
      cWtchdg++;    
    }
    HAL_TIM_IRQHandler(&TimHandle);
